...
export default class MainSreenSub2 extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    return (
      <Container>
        <ImageBackground source={BG_IMAGE} style={{ resizeMode: 'cover', width: Dimensions.get('window').width }}>
        <Header transparent>
          <Title style={{color: '#FFFFFF'}} >Coffee</Title>
        </Header>
        </ImageBackground>
        
        <Content contentContainerStyle={{ flexGrow: 1 }} scrollEventThrottle={300} onScroll={this.setCurrentReadOffset} removeClippedSubviews={true}>
          {this.renderItem()}
        </Content>
      </Container>
    );
